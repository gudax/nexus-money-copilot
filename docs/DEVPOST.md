...
- **Every number has a receipt.** The audit doesn't just stamp *verified* — tap any money-scale number in an answer and it opens the exchange field that proves it (source · field · matched value); a current price cites `price`/`close`, never an OHLC neighbour. Verification you can open, not a badge you have to trust.
